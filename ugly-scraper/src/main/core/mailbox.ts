...
const Mailbox = () => {
  ...
  const mailbox: MailboxAuthOptions[] = [] // get from db
  const conns: C[] = []
  ...
  const getAllMail = async (email: string) => {
    ...
    for await (const message of conn.fetch('1:*', { uid: true, envelope: true, source: true })) {
      mails.push(message)
    }

    await relinquishConnection(import.meta.env.MAIN_VITE_AUTHEMAIL)
    return mails
  }
  ...
  const newConnection = async (
    opts: MailboxAuthOptions,
    eventCallback?: (data: MBEventArgs) => Promise<void>
  ) => {
    ...
    try {
      return await client.connect().then(async () => {
        ...
        idx === -1 ? mailbox.push(opts) : mailbox.splice(idx, 1, opts)
  ...
  const getConnection = async (
    authDeets: MailboxAuthOptions,
    cb?: (data: MBEventArgs) => Promise<void>
  ) => {
...
export const accountToMailbox = (taskID: string, account: IAccount): MailboxAuthOptions => {
  return {
    auth: {
      user: import.meta.env.MAIN_VITE_AUTHEMAIL,
      pass: import.meta.env.MAIN_VITE_AUTHPASS
    },
    port: 993,
    secure: true,
...
const findIMAP = (taskID: string, email: string) => {
  switch (getDomain(email)) {
    ...
    default:
      throw new AppError(
        taskID,
        'domain not supported, please provide imap server e,g gmail = imap.gmail.com'
      )
  }
}
