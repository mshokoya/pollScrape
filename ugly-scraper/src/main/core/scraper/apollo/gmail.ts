...
const verifyGmail = async (taskID: string, browserCTX: BrowserContext, recoverEmail: string) => {
  ...
  const verificationMethods = await page
    .waitForSelector('div[class="vxx8jf"]', { visible: true, timeout: 5000 })
    .then(async () => {
  ...
  if (!verificationMethods || !verificationMethods.length)
    throw new AppError(taskID, 'failed to get verification methods')
  ...
  if (confirmRecovEmailIdx === -1)
    throw new AppError(taskID, 'failed to find "confirm email" recovery method')

  await verificationMethods[confirmRecovEmailIdx].click().then(() => {
    io.emit('apollo', { taskID, message: "selected the 'recovery email' verification method" })
  })

  const recovEmailInputEl = await page
    .waitForSelector('input[class="whsOnd zHQkBf"]', { visible: true, timeout: 5000 })
    .catch(() => null)
  if (!recovEmailInputEl) throw new AppError(taskID, 'failed to find recovery email input')

  await recovEmailInputEl.type(recoverEmail).then(() => {
    io.emit('apollo', { taskID, message: 'entered recovery email into field' })
  })
  ...
  await newButton.click().then(() => {
    io.emit('apollo', { taskID, message: 'click then next button' })
  })
}
...
export const visitGmailLoginAuthPortal = async (
  ...
  const gmailLoginButton = await page.$(
    'button[class="zp-button zp_zUY3r zp_n9QPr zp_MCSwB zp_eFcMr zp_grScD"]'
  )
  if (!gmailLoginButton)
    throw new AppError(taskID, 'failed to login, could not find google login button')
  await gmailLoginButton.click({ delay: 1000 }).then(async () => {
...
const gmailAuth = async (
  ...
  const emailField = await page.waitForSelector('input[class="whsOnd zHQkBf"][type="email"]', {
    visible: true,
    timeout: 10000
  })
  if (!emailField) throw new AppError(taskID, 'failed to login, could not input email')
  await emailField.type(account.email).then(() => {
    io.emit('apollo', { taskID, message: 'typed email into field' })
  })
  ...
  while (counter <= 5) {
    ...
    const gmailInvalidField = await page
      .$('input[class="whsOnd zHQkBf"][aria-invalid="true"]')
      .catch(() => null)
    const passwordField = await page
      .$('input[class="whsOnd zHQkBf"][type="password"]')
      .catch(() => null)
    ...
    if (gmailInvalidField) {
      ...
      await passwordField.type(account.password).then(() => {
        io.emit('apollo', { taskID, message: 'typed password into field' })
      })
      ...
      await nextButton.click({ delay: 1000 }).then(() => {
        io.emit('apollo', { taskID, message: 'clicked on next button' })
      })
      counter = 0
      ...
      if (!account.recoveryEmail) {
        throw new AppError(taskID, 'failed to login, recover email not provided')
      }
      await verifyGmail(taskID, browserCTX, account.recoveryEmail).then(() => {
        io.emit('apollo', { taskID, message: 'verified gmail account' })
      })
      counter = 0
      ...
      await confirmButton[1].click().then(() => {
        io.emit('apollo', { taskID, message: 'clicked on apollo confirmation button' })
      })
      counter = 0
...
// (FIX) complete func
export const apolloGmailLogin = async (
  taskID: string,
  browserCTX: BrowserContext,
  account: Partial<IAccount>
) => {
  ...
  await visitGmailLoginAuthPortal(taskID, browserCTX).then(() => {
    io.emit('apollo', { taskID, message: 'navigated to gmail auth portal' })
  })

  await gmailAuth(taskID, browserCTX, account).then(() => {
    io.emit('apollo', { taskID, message: 'prepared browser for login' })
  })
}

export const apolloGmailSignup = async (
  taskID: string,
  browserCTX: BrowserContext,
  account: Partial<IAccount>
) => {
  ...
  if (!gmailSignupButton)
    throw new AppError(taskID, 'failed to signup, could not find gmail signup button')

  await gmailSignupButton.click({ delay: 1000 }).then(() => {
    io.emit('apollo', { taskID, message: 'navigated to gmail auth portal' })
  })

  await gmailAuth(taskID, browserCTX, account).then(() => {
    io.emit('apollo', { taskID, message: 'completed gmail auth for signup' })
  })
}
